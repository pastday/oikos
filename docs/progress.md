...
화면 라벨만 `강의 분야` → **`전문분야`** / `Teaching areas` → **`Areas of Expertise`** 로 바꿨고,
**공개 페이지(A안·B안, ko·en)와 관리자 등록·수정 화면에서 같은 낱말을 쓴다.**
DB 필드명 `lectureFields` 는 그대로 두었다. 이름을 바꾸면 마이그레이션이 필요한데
얻는 것이 없다. 이 결정은 사용자 확인을 받았다.
...
### 6. 김동준 주임교수 상세 프로필 입력 (2026-08-24)

값의 **유일한 근거는 `docs/source/김동준소개.pptx`** 다. 31장 중 텍스트가 있는 슬라이드에서
기본사항·학력사항·전공 및 연구분야·석박사 학위·경력사항·자격 사항을 읽어 넣었다.
PPTX 에 없는 학력·경력·자격·수상·전문분야는 만들어 넣지 않았다.

| 필드 | 근거 슬라이드 | 넣은 것 |
| --- | --- | --- |
| `bioKo` `bioEn` | 2·3·13~15·22 | PPTX 사실만 조합한 2문단. 새 홍보 문구 없음 |
| `educationKo` `educationEn` | 2 (학력사항) + 3 (석·박사 학위) | 4개 학력 + 학위논문 2건 |
| `careerKo` `careerEn` | 3 (경력사항) + 23 (자격 사항 중 활동·직책) | 15줄. 슬라이드 안의 원래 순서 유지 |
| `lectureFieldsKo` `lectureFieldsEn` | 2 (전공 및 연구분야) | 호텔·외식·와인·호스피탈러티·관광·엔터테인먼트 |

**넣지 않은 것**과 이유:

- **생년월일·이메일·휴대전화** (슬라이드 2) — 개인정보. "교수 개인 연락처는 공개하지 않는다" 는
  기존 정책을 그대로 따랐다.
- **연구실적 논문 60여 편** (슬라이드 13~21), **간행물 8권** (슬라이드 22),
  **자격취득 6건** (슬라이드 23 중 와인소믈리에·커피바리스타·미국호텔협회 2종·레크리에이션
  지도사·KOV 국제와인기사작위), **기타 사업 계획 3건** (슬라이드 24)
  → **담을 필드가 없어 보류했다.** 억지로 경력에 섞지 않았다. 필요하면 필드를 추가한다.
- 슬라이드 4~12·25~31 은 사진 지면이라 제목 외에 텍스트가 없다.

**확인이 필요한 것**

- 소속 표기가 자료 안에서 갈린다. 슬라이드 2 는 `전, 영남이공대학교 호텔관광전공 교수`,
  슬라이드 3 은 `영남이공대학교 호텔·항공서비스 전공 교수` 다. 경력 필드에는 **경력사항
  슬라이드(3)의 표기를 그대로** 넣었다. `CLAUDE.md` 23항대로 임의로 하나를 고르지 않고 남겨 둔다.
- **영문 기관명은 PPTX 에 없다.** 대학·학회·기업 영문명을 보수적으로 옮겼을 뿐 공식 영문명이
  아니다. 교수 검수 때 확인해야 한다. FICB 도 PPTX 표기(`세계와인기사단총연합`)를 옮긴 것이다.
- 학위 종류를 `Ph.D.` / `M.A.` 로 특정하지 않았다. PPTX 는 박사·석사·학사라고만 적고 있다.
- `동인천고등학교` 는 PPTX 학력사항에 있어 그대로 넣었다. 대학원 교수 프로필에서 빼는 것이
  일반적이라면 CMS 에서 한 줄만 지우면 된다.

DB 는 김동준 교수 1건의 상세 프로필 8개 필드만 바꿨다. 기본정보(이름·영문명·직책·전공·
사진·`sortOrder`·`isPublished`)와 `createdAt` 은 그대로이고 다른 테이블도 건수가 같다.
...
- ~~교보문고 상품 페이지에 저자 소개가 없어 동일인 여부를 확인할 수 없다~~
  → **16단계에서 사용자 확인을 받아 해소.** 저서·기사 모두 김동준 주임교수 본인의 자료다.
  (상품 페이지에 저자 소개가 실려 있지 않은 것은 사실이므로 기록만 남긴다)
...
## 16단계에서 만든 것 (저서 · 언론 콘텐츠 확정 + UI 다듬기)

새 테이블도, 새 마이그레이션도 만들지 않았다. 15단계 구조를 그대로 쓰면서
**콘텐츠를 확정하고 화면을 다듬은** 단계다.

### 1. 저자 동일인 확인 — 해소

15단계에 남겨 두었던 "교보문고의 저자 `김동준` 이 우리 김동준 주임교수와 동일인인가" 는
**사용자 확인을 받았다.** 두 자료 모두 김동준 주임교수 본인의 것이다.
미결 목록 13번을 지웠고 seed 스크립트 주석에도 반영했다.

### 2. `seed:works` 를 upsert 로 바꿨다

15단계에는 **"있으면 건너뛴다"** 였다. 그래서 문구를 다듬어도 이미 들어간 행에는
반영되지 않았고, 고치려면 CMS 를 열거나 행을 지워야 했다.
16단계에서 이 스크립트를 **문구의 출처**로 삼기로 하고 찾으면 갱신하도록 바꿨다.

자연키는 **저서 = ISBN 또는 외부 링크**, **기사 = 외부 링크** 다.
저서에 ISBN 을 함께 본 이유는 링크가 서점에 따라 달라질 수 있어서다.

`sortOrder` 와 `isPublished` 는 **처음 만들 때만** 정하고 갱신할 때 손대지 않는다.
관리자가 순서를 바꾸거나 잠시 감춰 둔 것을 스크립트가 되돌리면 안 된다.

> ⚠️ 뒤집어 말하면 **관리자가 CMS 에서 다듬은 문구는 이 스크립트를 다시 돌리면 덮인다.**
> 운영에서 문구를 고쳤다면 이 스크립트를 돌리지 않거나 고친 내용을 스크립트에도 반영한다.

세 번 연속 실행해 `FacultyBook 1건 · FacultyArticle 1건` 이 유지되는 것을 확인했다.

### 3. 뷰 모델 — 한 줄로 합치던 것을 나눴다

15단계에는 저자·출판사·연도를 가운뎃점으로 이어 붙인 `meta` 한 줄이었다.
저자는 사람 이름이고 나머지는 책의 출처라 성격이 다른데, 한 줄에 붙이니 이름이 묻혔다.

| 이전 | 지금 |
| --- | --- |
| `FacultyBookView.meta` | `author` · `imprint`(출판사 · 연도) · `year` |
| `FacultyArticleView.meta` | `publisher` · `publishedOn` |

조립은 **`queries.ts` 한 곳에서 끝낸다.** A안·B안이 각자 이어 붙이면 두 화면의
표기가 갈리기 때문이다. 화면은 이미 만들어진 줄을 그리기만 한다.
`year` 를 따로 내보내는 것은 표지가 없을 때 글자 표지에 넣기 위해서다.

### 4. 화면 — 목록처럼 보이던 것을 카드로

**저서** — 표지 자리를 크게 잡고 오른쪽에 `저서` 표시 → 제목(세리프) → 저자 →
출판사·연도 → 소개 → [도서 보기] 를 세로로 세운다.

**기사** — **게시처와 게시일을 맨 위**에 작은 글씨로 나란히 세우고(사이에 세로 구분선)
그 아래 제목을 크게 둔다. 신문 기사 머리 부분과 같은 순서다.
15단계에는 제목이 먼저이고 메타가 아래여서 링크 목록처럼 보였다.

| | A안 | B안 |
| --- | --- | --- |
| 저서 | 카드. `xl` 부터 2열 | 상자 없이 지면. 표지에 그림자 |
| 기사 | 카드 1열 | 얇은 선(`divide-y`)으로만 나눈 지면 |
| 표지 없을 때 | navy + gold 괘선 + ivory 제목 + 연도 | midnight(BFrame) + bronze 괘선 + 제목 + 연도 |

**표지 placeholder 는 회색 네모가 아니다.** 다만 실제 표지로 오해되면 안 되므로
표지 그림을 흉내내지 않고 **가는 괘선 · 제목 · 발행연도** 셋만 앉혔다.
오른쪽 칸에 `저서` 표시가 이미 있어 표지 안에서 그 낱말을 되풀이하지 않는다.
손바닥만 한 폭에서 같은 낱말이 두 번 보이면 표지가 아니라 라벨처럼 읽힌다.
전체가 `aria-hidden` 이라 화면 읽기 프로그램이 제목을 두 번 읽지 않는다.

### 5. 라벨

| | 한국어 | 영어 |
| --- | --- | --- |
| 섹션 | 주요 저서 / 언론 · 미디어 | Selected Publications / Media & Press |
| 저서 표시 | 저서 | Selected Publication |
| 링크 | 도서 보기 / 기사 보기 | View Book / Read Article |

영어 링크 문구는 15단계의 `View book` · `Read article` 에서 **대문자 표기로 바꿨다.**

### 6. 기사 이미지 — 여전히 넣지 않는다

교수 사진을 기사 카드에 끌어다 쓰는 것도 하지 않았다. 지금 `Media` 가 0건이라
쓸 사진 자체가 없고, **교수 얼굴을 기사 썸네일 자리에 넣으면 그 사진이 그 기사에
실린 사진인 것처럼 읽힌다.** 글자 중심 카드로 두는 편이 정확하다.
사용권이 확인된 기사 이미지가 들어오면 CMS 에서 연결하면 그 자리에 나온다.

### 7. 데이터 대조

seed 전후로 다른 테이블은 **한 건도 달라지지 않았다.**

    Faculty 1 · FacultyBook 1 · FacultyArticle 1 · Program 2 · Course 33
    FAQ 8 · PageSection 19 · PageSectionItem 30 · Media 0 · SiteSetting 8

김동준 교수 행의 `updatedAt` 이 `2026-08-24T14:40:40.802Z` 그대로다.
이름·직책·전공·소개·학력·경력·전문분야·사진·`sortOrder`·`isPublished` 를
건드리지 않았다는 뜻이다.
...
> **해소됨 (16단계)** — 15단계에 13번으로 적어 두었던
> "교보문고 저자 `김동준` 이 우리 김동준 주임교수와 동일인인지 확인 필요" 는
> **사용자 확인을 받아 해소되었다.** 저서·기사 모두 김동준 주임교수 본인의 자료다.
