...
import { deleteCourse, saveCourse } from "../../../cms-actions";

export const metadata: Metadata = {
  title: "교과목 수정 | Oikos 관리자",
...
export default async function EditCoursePage({ params }: PageProps) {
  ...
  const action = saveCourse.bind(null, course.id);

  return (
    <div className="mx-auto flex max-w-5xl flex-col gap-6">
      <AdminPageHeader title="교과목 수정" description={course.titleKo}>
        <Link
          href="/admin/courses"
      ...
      <section className="rounded-lg border border-[#b3261e]/30 bg-[#b3261e]/[0.03] px-5 py-5">
        <h2 className="text-sm font-semibold text-[#b3261e]">교과목 삭제</h2>
        <p className="mt-1.5 mb-4 text-xs leading-relaxed text-muted">
          삭제하면 교육과정에서 즉시 사라지며 되돌릴 수 없습니다.
          잠시 감추려는 것이라면 위의 <strong>홈페이지에 공개</strong> 체크를 해제하세요.
        </p>
        <DeleteForm
          action={deleteCourse}
          id={course.id}
          confirmMessage={`"${course.titleKo}" 교과목을 삭제합니다. 되돌릴 수 없습니다. 계속할까요?`}
        />
      </section>
    </div>
  );
