...
/**
 * 여러 개의 파일을 **순서대로** 선택하는 입력. (자료실 첨부파일 등)
 *
 * ## 학교소식 `NewsAttachmentsField` 와의 관계
 *
 * 같은 목적의 컴포넌트가 학교소식에 먼저 있었다. 이 컴포넌트는 그 패턴을 그대로 따르되
 * **`name` 을 받아 재사용 가능하게** 하고, 배지에 확장자(`ext`)를 그대로 보여 준다.
 * (문서 파일이 많은 자료실에서 전부 "IMG" 로 보이면 안 되므로)
 *
 * 파일 자체는 업로드하지 않는다. **이미 `/admin/media` 에 올라간 파일 중에서** 고른다.
 * 값은 `name` hidden input **여러 개**로 나른다. 서버 액션이 `formData.getAll(name)` 으로 받는다.
 *
 * 모든 값이 이 컴포넌트의 state 에만 있으므로, 부모 폼이 저장 실패로 다시 그려져도
 * 선택 상태가 그대로 남는다. (React 19 form reset 문제 회피 — 학교소식과 같은 방식)
 */
export function MediaMultiSelect({
  ...
  /** hidden input 이름 (서버가 `formData.getAll(name)` 으로 읽는다) */
  name: string;
  label: string;
  hint: string;
  /** `/admin/media` 의 모든 파일 */
  ...
  return (
    <div className="flex flex-col gap-1.5">
      {selected.map((id) => (
        <input key={id} type="hidden" name={name} value={id} readOnly />
      ))}
